...
```
docker exec {SANDBOX_CONTAINER} <command>
```
...
```bash
docker exec {SANDBOX_CONTAINER} nmap -sn {target}
```

### Phase 2: Port Scan & Service Detection
On each discovered host, run:
```bash
docker exec {SANDBOX_CONTAINER} nmap -sV -sC -p {ports} <host_ip>
```

### Phase 3: Vulnerability Scan
On hosts with open web ports (80, 443, 8080), run:
```bash
docker exec {SANDBOX_CONTAINER} nuclei -u <host_ip> -t cves,vulnerabilities,misconfiguration -jsonl -silent -no-color
```
...
- NEVER run commands outside docker exec {SANDBOX_CONTAINER}
